/*!
 * \file test_ipc_queue.c
 * \date 2026-03-14
 * \authors Alessandro Bridi [ale.bridi15@gmail.com]
 *
 * \brief Test ipc_queue (Inter-Process Communication) functionality.
 */
...
/*!
 * \defgroup ipc_queue_api_read_and_process_all Tests for ipc_api_read_and_process_all function
 * \{
 */

void test_ipc_queue_input_queue_read_and_process_all_empty(void) {
...
void test_ipc_queue_input_queue_read_and_process_all_non_empty(void) {
...
void test_ipc_queue_input_queue_read_and_process_all_multiple(void) {
    struct InputsSharedEvent ev = { 0 };
    ipc_queue_api_push_event(ev, NULL);
    ipc_queue_api_push_event(ev, NULL);
    ipc_queue_api_push_event(ev, NULL);
...
void test_ipc_queue_input_queue_push_event_success(void) {
    struct InputsSharedEvent ev = { 0 };
    bool rc = ipc_queue_api_push_event(ev, NULL);
    TEST_ASSERT_EQUAL(true, rc);
}

void test_ipc_queue_input_queue_push_event_full(void) {
    struct InputsSharedEvent ev = { 0 };
...
void test_ipc_queue_input_queue_write_all_read_all(void) {
    struct InputsSharedEvent ev = { 0 };
    for (int i = 0; i < IPC_INPUT_QUEUE_SIZE - 1; i++) {
        bool rc = ipc_queue_api_push_event(ev, NULL);
        TEST_ASSERT_EQUAL_MESSAGE(true, rc, "Failed to push event to queue");
...
void test_ipc_queue_input_queue_write_100_read_100(void) {
    struct InputsSharedEvent ev = { 0 };
    for (int i = 0; i < 100; i++) {
        bool rc = ipc_queue_api_push_event(ev, NULL);
        TEST_ASSERT_EQUAL(true, rc);
...
int main() {
    ...
    RUN_TEST(test_ipc_queue_input_queue_read_and_process_all_empty);
    RUN_TEST(test_ipc_queue_input_queue_read_and_process_all_non_empty);
    RUN_TEST(test_ipc_queue_input_queue_read_and_process_all_multiple);
    RUN_TEST(test_ipc_queue_input_queue_read_and_process_read_event);

    RUN_TEST(test_ipc_queue_input_queue_push_event_success);
    RUN_TEST(test_ipc_queue_input_queue_push_event_full);

    RUN_TEST(test_ipc_queue_input_queue_write_all_read_all);
    RUN_TEST(test_ipc_queue_input_queue_write_100_read_100);
