 ...
 *
 * \brief Unit tests for screen_api_sync — the IPCUIData -> dashboard pump.
 *
 * \details screen_api_sync forwards each field of an IPCUIData snapshot through
...
/*!
 * \defgroup screen_api_sync Test argument-validation tests
 * \{
 */

void test_screen_api_sync_null_pointer(void) {
    enum ScreenReturnCode rc = screen_api_sync(NULL);
    TEST_ASSERT_EQUAL(SCREEN_RC_ERROR, rc);
}

void test_screen_api_sync_empty_snapshot_succeeds(void) {
    struct IPCUIData empty = { 0 };
    enum ScreenReturnCode rc = screen_api_sync(&empty);
...
/*!
 * \defgroup screen_api_sync No-op tests
 * \{
 */
...
void test_screen_api_sync_called_twice_keeps_dashboard_clean(void) {
    ...
    enum ScreenReturnCode rc = screen_api_sync(&snapshot);
